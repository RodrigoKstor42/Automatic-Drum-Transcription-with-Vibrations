...
class DataLoader(object):
    ...
    @classmethod
    def getCustomSplit(cls, folderPath=None, customDatasetRoot=None, cachePreprocessFolders=None, **kwargs):
        """
        Load a pre-split custom dataset with the structure:
        TRAIN/AUDIO, TRAIN/ANNOTATIONS, VAL/AUDIO, VAL/ANNOTATIONS, TEST/AUDIO, TEST/ANNOTATIONS.
        Audio and annotation files are paired by basename.
        """
        datasetRoot = customDatasetRoot or folderPath
        if datasetRoot is None:
            raise ValueError("custom_split requires folderPath or customDatasetRoot")

        datasetRoot = os.path.abspath(datasetRoot)
        ...
        for splitName in splitNames:
            splitAudioPaths, splitAnnotationPaths = cls._scanCustomSplit(datasetRoot, splitName)
            startIndex = len(audioPaths)
            audioPaths.extend(splitAudioPaths)
            annotationPaths.extend(splitAnnotationPaths)
            splitIndexes[splitName] = list(range(startIndex, len(audioPaths)))
            logging.info("custom_split %s tracks: %s", splitName, len(splitAudioPaths))

        if not audioPaths:
            raise ValueError("custom_split found no paired WAV/TXT files under {}".format(datasetRoot))
        ...
        dataset = cls(
            ...
            cachePreprocessFolders=cachePreprocessFolders,
            checkFilesNameMatch=False,
            crossValidation=False,
            **kwargs,
        )
        dataset.trainIndexes = np.array(splitIndexes["TRAIN"])
        dataset.valIndexes = np.array(splitIndexes["VAL"])
        dataset.testIndexes = np.array(splitIndexes["TEST"])
        dataset.groups = [config.getFileBasename(path) for path in dataset.audioPaths]
        dataset.keptGroups = dataset.groups

        return dataset

    @classmethod
    def _scanCustomSplit(cls, datasetRoot, splitName):
        audioFolder = os.path.join(datasetRoot, splitName, "AUDIO")
        annotationFolder = os.path.join(datasetRoot, splitName, "ANNOTATIONS")

        if not os.path.isdir(audioFolder):
            logging.warning("custom_split %s missing AUDIO folder: %s", splitName, audioFolder)
            return [], []
        if not os.path.isdir(annotationFolder):
            logging.warning("custom_split %s missing ANNOTATIONS folder: %s", splitName, annotationFolder)
            return [], []

        audioByBaseName = {
            config.getFileBasename(path): path
            for path in sorted(glob.glob(os.path.join(audioFolder, "*")))
            if os.path.splitext(path)[1].lower() == ".wav"
        }
        annotationByBaseName = {
            config.getFileBasename(path): path
            for path in sorted(glob.glob(os.path.join(annotationFolder, "*")))
            if os.path.splitext(path)[1].lower() == ".txt"
        }

        missingAnnotations = sorted(set(audioByBaseName) - set(annotationByBaseName))
        missingAudio = sorted(set(annotationByBaseName) - set(audioByBaseName))
        for basename in missingAnnotations:
            logging.warning("custom_split %s orphan audio without annotation: %s", splitName, audioByBaseName[basename])
        for basename in missingAudio:
            logging.warning("custom_split %s orphan annotation without audio: %s", splitName, annotationByBaseName[basename])

        pairedBaseNames = sorted(set(audioByBaseName) & set(annotationByBaseName))
        return [audioByBaseName[name] for name in pairedBaseNames], [annotationByBaseName[name] for name in pairedBaseNames]
    ...
    def factoryMixedDatasets(cls, folderPath: str, testFold=0, scenario="all", regulateSamplingProbability=1, dataAugmentation=False, **kwargs):
        ...
        elif scenario == "custom_split":
            trainingSets = "custom_split"
            validationSets = "custom_split"
        elif scenario == "all":
