...
for _legacy_name in ("MutableSequence", "MutableMapping", "Mapping", "Sequence", "Iterable"):
    if not hasattr(collections, _legacy_name):
        setattr(collections, _legacy_name, getattr(collections.abc, _legacy_name))

for _legacy_name, _replacement in (("float", float), ("int", int), ("complex", complex)):
    if not hasattr(np, _legacy_name):
        setattr(np, _legacy_name, _replacement)
